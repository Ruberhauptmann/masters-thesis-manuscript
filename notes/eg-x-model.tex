...
\begin{align}
    ...
    \mathrm{M} &= \frac{\pi}{3 a_0} \begin{pmatrix} 1 \\ \sqrt{3} \end{pmatrix} \\
    \mathrm{K} &= \frac{2\pi}{3 a_0} \begin{pmatrix} 1 \\ - \frac{1}{\sqrt{3}} \end{pmatrix}
\end{align}
...
With that, we can calculate \todo{Do the calculation right!}:
\begin{align}
    f_{\mathrm{X}} (\vb{k}) &= -\frac{t_X}{2} \sum_{\vb{\delta}_{\mathrm{X}}} e^{\iu \vb{k} \vb{\delta}_{\mathrm{X}}}
    = -\frac{t_X}{2} \left( e^{\iu \sqrt{3} a_0 (\frac{\sqrt{3}}{2} k_x + \frac{1}{2} k_y)}
    + e^{\iu \sqrt{3} a_0 (\frac{\sqrt{3}}{2} k_x - \frac{1}{2} k_y)}
    + e^{\iu \sqrt{3} a_0 (-k_y)} \right.  \\
    &+ \left. e^{\iu \sqrt{3} a_0 (-\frac{\sqrt{3}}{2} k_x - \frac{1}{2} k_y)}
    + e^{\iu \sqrt{3} a_0 (-\frac{\sqrt{3}}{2} k_x + \frac{1}{2} k_y)}
    + e^{\iu \sqrt{3} a_0 k_y} \right) \\
    &= -\frac{t_X}{2} \left( 2 \cos{(\sqrt{3} a_0 k_y)} + 2 e^{\iu \sqrt{3} a_0 (\frac{\sqrt{3}}{2} k_x)} \cos{(\frac{\sqrt{3}}{2} a_0 k_y)} \right. \\
    &+ \left. 2 e^{-\iu \sqrt{3} a_0 (\frac{\sqrt{3}}{2} k_x)} \cos{(\frac{\sqrt{3}}{2} a_0 k_y)} \right) \\
    &= -t_X \left( \cos{(\sqrt{3} a_0 k_y)} + 2 \cos{(\frac{\sqrt{3}}{2} a_0 k_y)} \cos{(\frac{3}{2} a_0 k_x)} \right)
\end{align}
We can do the same for the hopping between Graphene sites, for example :
\begin{align}
    -t_{\mathrm{Gr}} \sum_{\langle ij \rangle, \sigma \sigma^{\prime}} c_{i, \sigma}^{(B), \dagger} c_{j, \sigma^{\prime}}^{(A)}
    &= -\frac{t_{\mathrm{Gr}}}{2} \sum_{i, \sigma \sigma^{\prime}} \sum_{\vb{\delta}_{AB}} c_{i, \sigma}^{(B), \dagger} c_{i + \vb{\delta}_{AB} , \sigma^{\prime}}^{(B)} \\
    &= -\frac{t_{GR}}{2} \sum_{\vb{k}, \sigma, \sigma^{\prime}}  c_{\vb{k}, \sigma}^{(A) \dagger} c_{\vb{k}, \sigma^{\prime}}^{(B)} \sum_{\vb{\delta}_{BA}} e^{\iu \vb{k} \vb{\delta}_{BA}}
\end{align}
We note
\begin{align}
    \sum_{\vb{\delta}_{AB}} e^{\iu \vb{k} \vb{\delta}_{AB}} = \left( \sum_{\vb{\delta}_{BA}} e^{\iu \vb{k} \vb{\delta}_{BA}} \right)^*
\end{align}
and calculate \todo{Do the calculation right!}
\begin{align}
    f_{Gr} = -\frac{t_{Gr}}{2} \sum_{\vb{\delta}_{AB}} e^{\iu \vb{k} \vb{\delta}_{AB}} =
\end{align}

Doing the same for the hoppings between the \(A\) and \(B\) Graphene sites as well as the hybridisaton between \(\mathrm{X}\) and Graphene \(A\) lattice sites,
