...
\todo{I dont know why that is. Can I support that somehow better?}
...
\todo{Clear up nomenclature: BCS theory? BdG theory? Mean field theory?}
...
This descriptions is based on a one-particle wave function \(\phi (x)\) \todo{Finish up London theory}
...
Description is based on the fact that the Fermi sea is unstable towards development of bound pairs under arbitrarily small attraction \cite{cooperBoundElectronPairs1956} \todo{Better explanation microscopic OP}

BCS theory gave a microscopic explanation for the wavefunction used in the phenomenological London theory.
Furthermore, it is the order parameter in the developing GL-theory of phase transitions \cite{landauTheorySuperconductivity1965}.
As explained in \cref{sec:Ginzburg-Landau theory of superconductivity}, finite order parameter carry a superflow.

The BCS-Hamiltonian is: \todo{BCS Hamiltonian}
\begin{equation}
	H_{BCS} = 
\end{equation}
...
\todo{How can BCS theory be taken as a mean-field theory?}
...
In this work: extension to multiple orbitals (on the honeycomb lattice) and an attractive interaction, i.e. a negative \(U\).
...
\begin{equation}
	H_{\mathrm{int}} = -U \sum_{i, \alpha} c_{i, \alpha, \uparrow}^{\dagger} c_{i, \alpha, \downarrow}^{\dagger} c_{i, \alpha, \downarrow} c_{i, \alpha, \uparrow}
\end{equation}
where \(\alpha\) counts orbitals and the minus sign in front is taken so that \(U > 0\) now corresponds to an attractive interaction.
